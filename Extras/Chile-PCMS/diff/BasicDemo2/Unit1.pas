...
procedure TFDiff.FormActivate(Sender: TObject);
begin
...
procedure TFDiff.FormDestroy(Sender: TObject);
begin
  source1.Free;
  source2.Free;
  result1.Free;
...
procedure TFDiff.Clear(left, right: boolean);
begin
  if left then
  begin
    source1.Clear;
...
procedure TFDiff.BuildHashList(left,right: boolean);
var
  i: integer;
begin
  if left then
...
procedure TFDiff.OpenFile1(const filename: string);
var
  i: integer;
begin
...
procedure TFDiff.OpenFile2(const filename: string);
var
  i: integer;
begin
  if not fileExists(fileName) then exit;
...
procedure TFDiff.Open11Click(Sender: TObject);
begin
  if OpenDialog1.Execute then OpenFile1(OpenDialog1.FileName);
end;
//------------------------------------------------------------------------------

procedure TFDiff.Open21Click(Sender: TObject);
begin
  if OpenDialog1.Execute then OpenFile2(OpenDialog1.FileName);
end;
//------------------------------------------------------------------------------

procedure TFDiff.Exit1Click(Sender: TObject);
begin
  close;
end;
//------------------------------------------------------------------------------

procedure TFDiff.mnuCompareClick(Sender: TObject);
var
  i: integer;
begin
  ...
  try
    ...
    begin
      ...
      StatusBar1.Panels[2].Text := ' Faltantes: ' + inttostr(adds);
      StatusBar1.Panels[3].Text := ' Sobrantes: ' + inttostr(deletes);
    end;
...
procedure TFDiff.mnuCancelClick(Sender: TObject);
begin
  Diff.Cancel;
end;
//------------------------------------------------------------------------------

procedure TFDiff.FormResize(Sender: TObject);
var
  i: integer;
begin
  with ResultGrid do
...
procedure TFDiff.ResultGridDrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
const
...
procedure TFDiff.mnuIgnoreCaseClick(Sender: TObject);
begin
...
procedure TFDiff.mnuIgnoreWhiteSpaceClick(Sender: TObject);
begin
...
procedure TFDiff.PreviousChanges1Click(Sender: TObject);
var
  row: integer;
  Kind: TChangeKind;
begin
...
procedure TFDiff.NextChanges1Click(Sender: TObject);
var
  row: integer;
  Kind: TChangeKind;
begin
