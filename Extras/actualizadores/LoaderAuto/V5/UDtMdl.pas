...
type
  ...
  private
    ...
    { Contiene información a cerca de las tablas leidas desde archivo de parámetros }
    { Declarado aquí para que se destruya cuando se destruye el data module }
...
begin
  { En caso de desconexión (SQLCode = -901) abortar. Aqui el servicio de Interbase se cayó.
    El error -902 hace referencia a una desconexión pero se nuestra sesión. El servicio de Interbase
    está activo. En ambos casos no rollback ya que no hay conexión }
  ...
  if not Abort then
    ...
    begin
      ...
      try
        ...
        { Actualizar las líneas que se procesaron bien y las que con errores }
        Commit(TRNAll);
      except
        on E: EDataBaseError do
        begin
