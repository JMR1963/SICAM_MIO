...
// WSDL     : http://172.16.0.31/cgi-bin/wsrestriccioncentral/wsrestriccioncentral/wsdl/IWsListaControl
// Encoding : utf-8
// Version  : 1.0
// (22/07/2015 19:37:32 - 1.33.2.5)
// ************************************************************************ //

unit IWsListaControl1;

interface

uses soap.InvokeRegistry, soap.SOAPHTTPClient, system.Types, soap.XSBuiltIns, SqlOracleManage, uglobal;
...
type
  ...
  XML_CONSULTA         = class;                 { "urn:https://www.dnmservices.gov.ar/wsConarc/wsConarc.php" }
  TESTATSOURCE         = class;                 { "urn:WsListaControlIntf" }
  Restriccion          = class;                 { "urn:WsListaControlIntf" }
  TRESULTADO           = class;                 { "urn:WsListaControlIntf" }



  // ************************************************************************ //
  // Namespace : urn:https://www.dnmservices.gov.ar/wsConarc/wsConarc.php
  // ************************************************************************ //
  XML_CONSULTA = class(TRemotable)
  private
    FFUENTES: WideString;
    FUSUARIO: WideString;
    FCLAVE: WideString;
  ...
  TESTATMANYSOURCES = array of TESTATSOURCE;    { "urn:WsListaControlIntf" }
  ...
  XML_RESPUESTA = array of Restriccion;         { "urn:https://www.dnmservices.gov.ar/wsConarc/wsConarc.php" }
  ...
  private
    Frestricciones: XML_RESPUESTA;
    Ffuentes: TESTATMANYSOURCES;
  public
    destructor Destroy; override;
  published
    property restricciones: XML_RESPUESTA read Frestricciones write Frestricciones;
    property fuentes: TESTATMANYSOURCES read Ffuentes write Ffuentes;
  end;


  // ************************************************************************ //
  // Namespace : urn:WsListaControlIntf-IWsListaControl
  // soapAction: urn:WsListaControlIntf-IWsListaControl#ConsultaRestriccion
  // transport : http://schemas.xmlsoap.org/soap/http
  // style     : rpc
  // binding   : IWsListaControlbinding
  // service   : IWsListaControlservice
  // port      : IWsListaControlPort
  // URL       : http://172.16.0.31/cgi-bin/wsrestriccioncentral/wsrestriccioncentral/soap/IWsListaControl
  // ************************************************************************ //
  IWsListaControl = interface(IInvokable)
  ['{77582481-D2F7-ED01-5655-E27ACBB032BD}']
    function  ConsultaRestriccion(const Consulta: XML_CONSULTA): TRESULTADO; stdcall;
  end;

function GetIWsListaControl( UseWSDL: Boolean=System.False; Addr: string=''; HTTPRIO: THTTPRIO = nil): IWsListaControl;   //      ip : string;


implementation

function GetIWsListaControl(UseWSDL: Boolean; Addr: string; HTTPRIO: THTTPRIO): IWsListaControl;  // ip : string;
const
//  defWSDL = 'http://172.16.0.31/cgi-bin/wsrestriccioncentral/wsrestriccioncentral/wsdl/IWsListaControl';
//  defURL  = 'http://172.16.0.31/cgi-bin/wsrestriccioncentral/wsrestriccioncentral/soap/IWsListaControl';
  defSvc  = 'IWsListaControlservice';
  defPrt  = 'IWsListaControlPort';
VAR  RIO: THTTPRIO;
     defWSDL,defURL : AnsiString;
begin

  defWSDL := 'http://'+uglobal.url  +'/cgi-bin/wsrestriccioncentral/wsrestriccioncentral/wsdl/IWsListaControl';
  defURL  := 'http://'+uglobal.url+'/cgi-bin/wsrestriccioncentral/wsrestriccioncentral/soap/IWsListaControl';
  Result := nil;
  if (Addr = '') then
  begin
    if UseWSDL then
...
initialization
  InvRegistry.RegisterInterface(TypeInfo(IWsListaControl), 'urn:WsListaControlIntf-IWsListaControl', 'utf-8');
  InvRegistry.RegisterDefaultSOAPAction(TypeInfo(IWsListaControl), 'urn:WsListaControlIntf-IWsListaControl#ConsultaRestriccion');
  RemClassRegistry.RegisterXSClass(XML_CONSULTA, 'urn:https://www.dnmservices.gov.ar/wsConarc/wsConarc.php', 'XML_CONSULTA');
  RemClassRegistry.RegisterXSClass(TESTATSOURCE, 'urn:WsListaControlIntf', 'TESTATSOURCE');
  RemClassRegistry.RegisterXSInfo(TypeInfo(TESTATMANYSOURCES), 'urn:WsListaControlIntf', 'TESTATMANYSOURCES');
  RemClassRegistry.RegisterXSClass(Restriccion, 'urn:WsListaControlIntf', 'Restriccion');
  RemClassRegistry.RegisterXSInfo(TypeInfo(XML_RESPUESTA), 'urn:https://www.dnmservices.gov.ar/wsConarc/wsConarc.php', 'XML_RESPUESTA');
  RemClassRegistry.RegisterXSClass(TRESULTADO, 'urn:WsListaControlIntf', 'TRESULTADO');
