...
uses
  ...
  IwsPuentePRY1, IwsPuenteBolivia1, Ipuentechileno2, md5, WinApi.ShellAPI;
...
begin
  ...
  try
    Cantidad := 10000;
    if (UpperCase(organismo) = 'ANSES') OR (UpperCase(organismo) = 'MINDESASOC') then
      Cantidad := 100000;

    if ((UpperCase(organismo) = 'ANSES') OR (UpperCase(organismo) = 'MINDESASOC')) and (FileSize(path) <= 23068672) then
      Result := False
    else
    begin
...
//grabarEnArchivoLog('1');
    ...
    begin
      ...
      log.Add('Error al guardar el manifiesto - ' + strfechahora + e.Message);
      log.SaveToFile(logpath + '\' + formatdatetime('yyyymmddhhnnss', now) + '.txt');
      log.Clear;
