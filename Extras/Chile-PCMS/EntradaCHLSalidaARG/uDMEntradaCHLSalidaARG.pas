unit uDMEntradaCHLSalidaARG;

interface

uses
  SysUtils, Classes, DB, IBCustomDataSet, IBQuery, IBDatabase, RxMemDS,NetConn,
  DBClient;
...
type
  TDMSem = class(TDataModule)
    ...
    tblArchApellidos: TStringField;
    tblArchNombres: TStringField;
    tblArchFechaNac: TStringField;
    tblArchNroDOC: TStringField;
    tblArchNroLibreta: TStringField;
    tblArchEMISORDOC: TStringField;
    tblArchFECHA_HORA: TStringField;
    tblArchgenero: TStringField;
    tblArchtdi: TStringField;
    strngfldArchorigen: TStringField;
...
begin
  ...
  try
    ...
    GLOBAL_SemPath := inifile.ReadString('SEMAFORO','PATH','c:\test');
    GLOBAL_SERVER :=  inifile.ReadString('SERVER','NOMBRE_PUERTO','');
    ...
    GLOBAL_WSValSemaforo := inifile.ReadString('WS','URL_ValSemaforo','http://10.170.140.7:82/cgi-bin/WSValidarSemaforo/WSValidarSemaforo/wsdl/IWSValidarSemaforo');
  finally
    inifile.Free;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      tblArch.FieldByName('cont').AsInteger := GLOBAL_CONTADOR;
      tblArch.FieldByName('Apellidos').AsString := valorNodoXML(cont,'APELLIDO1')+' '+valorNodoXML(cont,'APELLIDO2');
      tblArch.FieldByName('Nombres').AsString :=   valorNodoXML(cont,'NOMBRE1')+' '+valorNodoXML(cont,'NOMBRE2');
      tblArch.FieldByName('FechaNac').AsString := valorNodoXML(cont,'FECHANAC');
      tblArch.FieldByName('NroDOC').AsString := valorNodoXML(cont,'NUMDOC');
      tblArch.FieldByName('GENERO').AsString := valorNodoXML(cont,'GENERO');
      tblArch.FieldByName('EMISORDOC').AsString := valorNodoXML(cont,'EMISORDOC');
      tblArch.FieldByName('FECHA_HORA').AsString := valorNodoXML(cont,'FECHA_HORA');
      tblArch.FieldByName('NroLibreta').AsString := valorNodoXML(cont,'NROLIBRETA');
      tblArch.FieldByName('genero').AsString := valorNodoXML(cont,'GENERO');
      tblArch.FieldByName('tdi').AsString := valorNodoXML(cont,'tipodoc'); //como lo tenes? en el archivo
      tblArch.FieldByName('origen').AsString := valorNodoXML(cont,'origen');
      tblArch.Post;
    end;
  end;
