...
uses vcl.Forms , System.Classes, Data.DB, DBAccess, Ora,MemDS, XML.XMLIntf, System.StrUtils, WsListaControlIntf,
     ActiveX, System.IniFiles, uglobal, Vcl.dialogs, Soap.SOAPHTTPClient, ADODB, IpuenteUry1;
...
type
  ...
  private
   ...
   procedure generarTxT2(path, resultado:string);
    ...
    function consultoPorDOC(doc: string) : boolean;
  ...
  public
    ...
    procedure HTTPRIO1BeforeExecute(const MethodName: String;   Request: TStream);
...
procedure TSqlOracle.HTTPRIO1BeforeExecute(const MethodName: String;
  Request: TStream);
...
begin
      ...
      Result[i].OBSERVACIONES         :=  UpperCase('Previo a autorizar el Movimiento Migratorio, comunicarse al Departamento de Información, Asistencia y Cooperación (DIAC) a los teléfonos 011-4874-2323 o 011-4874-2324');
...
begin
      ...
      //Result[i].OBSERVACIONES         :=    UpperCase('Tomar contacto con INTERPOL al tel: 011-43465753 / Cel 011-1545280662 o 011-1531970914 - poseen whatsapp. Además enviar mail a cnc@interpol.gov.ar.');
      Result[i].OBSERVACIONES         :=    UpperCase('Sujeto con perfil seleccionado. Deberá ajustarse a las instrucciones impartidas en la circular vigente.');
...
function TSqlOracle.consultoPorDOC(doc: string) : boolean;
begin
  Result := false;
  with  oraQuery2 do
  begin
...
begin
  try
    ...
    begin
      if consultoPorDOC(Individuo.NUMDOC) then
      begin
      ...
      begin
        if consultoPorDOC(Individuo.NUMEROIDENIFICACION) then
        begin
          Resultado:=  StrFPEresultadoConsulta(Individuo);
          ForceDirectories('.\'+formatdatetime('yyyy',now)+'\'+formatdatetime('mm',now)+'\'+formatdatetime('dd',now));
          generarTxT2('.\'+formatdatetime('yyyy',now)+'\'+formatdatetime('mm',now)+'\'+formatdatetime('dd',now)+'\logProcesoHSPD6.txt',formatdatetime('dd/mm/yyyy HH:mm:ss',now)+' - '+ UpperCase(Individuo.NUMDOC)+' - '+UpperCase(Individuo.FECHANAC)+' - '+  UpperCase(Individuo.APELLIDO1)+' '+ UpperCase(Individuo.NOMBRE1)+' - '+ UpperCase(Individuo.PASO))
    ...
    Error:= true;
    generarTxT2('.\logErrorHSPD6.txt',formatdatetime('dd/mm/yyyy HH:mm:ss',now)+' - '+ UpperCase(Individuo.NUMDOC)+' - '+UpperCase(Individuo.FECHANAC)+' - '+  UpperCase(Individuo.APELLIDO1)+' '+ UpperCase(Individuo.NOMBRE1)+' - '+ UpperCase(Individuo.PASO));
    {ForceDirectories('.\'+formatdatetime('yyyy',now)+'\'+formatdatetime('mm',now)+'\'+formatdatetime('dd',now));
...
begin
  try
    ...
    cons.pais:=Individuo.NACIONALIDAD; // Nac o Pais emisor
...
begin
  ...
  try
    ...
    begin
      ...
      if (Trim(Individuo.APELLIDO2) <> '') and (Trim(individuo.NOMBRE2) <> '') then
      begin
        parametros :=4;
...
begin
  ...
  try
    ...
    begin
      ...
      if (Trim(APELLIDO2) <> '')  and (Trim(NOMBRE2) <> '') then
      begin
        parametros :=4;
        consulta := consulta + ' and Apellido2 LIKE :unApellido2 ||''%'' ';
        consulta := consulta + ' and nombre2 LIKE :unNombre2 ||''%'' ';
...
begin
  ...
  if doc <> '' then
  BEGIN
    res := dameNombreApellidosYdoc(doc,individuo.APELLIDO1, individuo.APELLIDO2, individuo.NOMBRE1, individuo.NOMBRE2 );
...
begin
  ...
  begin
    Result.COD_RTA := '0';
    Result.COD_DETALLE_RTA := '1'; // Faltan datos básicos para la búsqueda
  end;
end;
